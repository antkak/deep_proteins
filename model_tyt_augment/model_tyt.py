...
class DataGen(Sequence):
    def __init__(self, x_set, y_set, batch_size, augmentations=False):
        self.x, self.x_pr, self.y = x_set[0], x_set[1], y_set
    ...
    def __getitem__(self, idx):
        ...
        batch_x_pr = self.x_pr[idx * self.batch_size:(idx + 1) * self.batch_size,:,:]
        batch_y = self.y[idx * self.batch_size:(idx + 1) * self.batch_size,:,:]
        if self.augment:
            p = 0.15
            for i in range(batch_x.shape[0]):
                for j in range(batch_x.shape[1]):
                    # with prob = p, if a residue exists (not noSec), change residue
                    if np.random.binomial(1,0.15)==1 and int(batch_x[i,j,0])!=1:
                        pos = np.random.randint(1,high=22)
                        batch_x[i,j,:] = np.zeros(22)
                        batch_x[i,j,pos] = 1

        return [batch_x,batch_x_pr], batch_y
...
#         # with prob = p, if a residue exists (not noSec), change residue
#         if np.random.binomial(1,0.15)==1 and int(X_train[0][i,j,0])!=1:
#             pos = np.random.randint(1,high=22)
#             X_train[0][i,j,:] = np.zeros(22)
#             X_train[0][i,j,pos] = 1
...
history, model = train_gen(X_train, y_train, X_val=X_val, y_val=y_val)
