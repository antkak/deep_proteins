...
columns = ["id", "len", "input", "profiles", "expected", "sa"] # xxx add sa
...
def get_data(arr, bounds=None):
    ...
    for i in bounds:
        ...
        data[i] = [str(i+1), len(seq), seq, np.array(profiles), q8, np.array(sas)] # xxx add sa
...
train_target_sa = \
    train_df[['sa']][(train_df.len.astype(int) <= maxlen_seq)].values
# print(train_target_sa.shape)
# print(train_target_sa[0][0].shape)
# # print(np.array([x[0] for x in train_target_sa])[5000:5300,:])
# assert(False)
...
def run_test_single_input(_model, data1, data2, csv_name, npy_name):
    ...
    # Get predictions using our model
    y_test_pred, _ = _model.predict([data1,data2])
...
# Extract labels for solvent accessibility (multitask)
abs_solv = []
rel_solv = []
...
y_train_solv = np.concatenate((abs_solv_out,rel_solv_out), axis = 2)
...
y_train_solv  = y_train_solv[randomize]
...
y_train = [train_target_data[vn:,:,:], y_train_solv[vn:,:,:]]
X_val =   [train_input_data[:vn,:,:], train_profiles_np[:vn,:,:]]
y_val =   [train_target_data[:vn,:,:], y_train_solv[:vn,:,:]]
